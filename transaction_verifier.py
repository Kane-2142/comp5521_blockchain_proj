...
class TransactionVer_Exception(Exception):
    ...
    def validate(self):
        ...
        for tx_input in self.inputs:
            ...
            try:
                ...
                raise TransactionVer_Exception(f"{transaction_hash}:{output_index}", "Could not find locking script for utxo")
            try:
                self.execute_script(tx_input["unlocking_script"], locking_script)
                self.is_valid = True
            except Exception:
                logging.info('Transaction script validation failed')
                raise TransactionVer_Exception(f"UTXO ({transaction_hash}:{output_index})", "Transaction script validation failed")
